; output/00001756.smt: generated with canalyze.
; ($2 > $cast: size_t (($1 - $0) / 1(S64))$) Assumption: true
(declare-const $2 Int)
...
(assert (> $2 (/ (- $1 $0) 1)))
; (($cast: long $4$ - (1(S64) * (0(S64) + $cast: long $cast: size_t (($1 - $0) / 1(S64))$$))) >= 1(S64)) Assumption: true
(declare-const $4 Int)
(assert (and (< 0 $4) (< $4 18446744073709551615)))
(assert (>= (- $4 (* 1 (+ 0 (/ (- $1 $0) 1)))) 1))
; $3 Assumption: true
(declare-const $3 Int)
(assert (and (< (- 9223372036854775808) $3) (< $3 9223372036854775807)))
(assert (not (= $3 0)))
; ($cast: size_t (($1 - $0) / 1(S64))$ < $2) Assumption: true
(assert (< (/ (- $1 $0) 1) $2))
