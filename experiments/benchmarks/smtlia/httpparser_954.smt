; output/00001753.smt: generated with canalyze.
; ($2 > $cast: size_t (($1 - $0) / 1(S64))$) Assumption: false
(declare-const $2 Int)
(assert (and (< 0 $2) (< $2 18446744073709551615)))
(declare-const $1 Int)
(assert (and (< (- 9223372036854775808) $1) (< $1 9223372036854775807)))
(declare-const $0 Int)
(assert (and (< (- 9223372036854775808) $0) (< $0 9223372036854775807)))
(assert (not (> $2 (/ (- $1 $0) 1))))
; (($cast: long $4$ - (1(S64) * (0(S64) + $cast: long ($2 - 1(U64))$))) >= 1(S64)) Assumption: true
(declare-const $4 Int)
(assert (and (< 0 $4) (< $4 18446744073709551615)))
(assert (>= (- $4 (* 1 (+ 0 (- $2 1)))) 1))
; $3 Assumption: true
(declare-const $3 Int)
(assert (and (< (- 9223372036854775808) $3) (< $3 9223372036854775807)))
(assert (not (= $3 0)))
; ($cast: size_t (($1 - $0) / 1(S64))$ < $2) Assumption: false
(assert (not (< (/ (- $1 $0) 1) $2)))
; ($2 > 0(U64)) Assumption: true
(assert (> $2 0))
; [sym$FuncSymbol:1 memchr$] == NULL Assumption: false
(assert (not (= $1 0)))
; $0 Assumption: true
(assert (not (= $0 0)))
(check-sat)
